...
int main(int argc, char* argv[]) {
	...
	if (argc < 3)
	{
		printf("Expect 3 arguments, passed %d.\n", argc);
	} else 
	{
	...
	assert(pdf_actual < 0xFFFFF00);
        ...
        unsigned int seed = (unsigned int) (ts.tv_sec*1000000000+ts.tv_nsec);
	seed = seed;
	...
	{
		printf("pre_computed_generations[%d]: %10u\n", i, pre_computed_generations[i]);
	}
	for (register int i = num_generations-4; i < num_generations; i++)
	{
		printf("pre_computed_generations[%d]: %10u\n", i, pre_computed_generations[i]);
	}
	...
	for (register int i = 0; i < (int) pdf_padded; i++) { pdf[i] = 0.0f; }
	for (register int i = 0; i < (int) pdf_actual; i++) { pdf[i] = (float) 1/_dummy; }
	...
	for (register int i = 0; i < (int) res_padded; i++) { res[i] = 0.0f; }
